...
When a securelevel is positive, the kernel disallows even root the ability to perform certain tasks
...
So some important boxes (pwsafe, firewalls) can have a different, hopefully smaller set of vulnerabilites.
...
database of information security vulnerabilites?(
...
`tar -cvzf balls.tar.gz /path/to/compress`

1. What is strace for?

Allowing you to see what system calls a program is making
...
To be able to remotely manage just that server
...
VPN - enables a remote computer to communicate with a network as if it were on that network. It provides security through encryption(2)

1. What is port knocking?

Sending packets to a series of closed ports on a firewall that opens a previously closed port
