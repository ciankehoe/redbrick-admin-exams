...
1. When running a Docker container, how would you bind port 22000 on the host to port 80 of the container?
...
1. What is the difference between ‘docker attach’ and ‘docker exec’?
...
- portsnap fetch \$PACKAGE - portsnap extract \$PACKAGE

1. What is 'dtrace' and what does it do?

- dtrace is a tracing utility found on BSD. - It allows admins to troubleshoot and display information on the performace
...
5. Enforce SSH key login Change `PasswordAuthentication yes` to
   `PasswordAuthentication no`, then restart ssh `sudo service ssh restart`.
6. Create the `haxor` user
```
sudo adduser haxor
```
...
Su switches your account to another user requiring the users loging when no args are passed it defaults to the root user. Sudo runs a single command with root privileges – it doesn't switch to the root user or require a separate root user password.
...
because all of these are unencrypted
